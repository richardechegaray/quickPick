...
module.exports = {
  ...
  checkFB(req, res, next) {
    ...
    let url =
      "https://graph.facebook.com/debug_token?input_token=${fbToken}&access_token=${process.env.FB_APP_ID}|${process.env.FB_APP_SECRET}";
    ...
    axios
      ...
      .then((fbResponse) => {
        if (fbResponse.data.data.is_valid) {
          ...
          res.status(401);
          res.json({ message: "Invalid token!", ok: false });
        }
      })
      .catch((error) => {
        ...
        res.status(401);
        console.log(error);
        res.json({ message: "FB request returned an error", ok: false });
      });
  },
};
